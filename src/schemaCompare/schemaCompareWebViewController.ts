...
> {
    ...
    private convertExcludesToObjectIds(
        ...
        // Iterate through Map entries using both key and value
        excludedDiffEntries.forEach((value: mssql.DiffEntry, key: string) => {
            // Use the diff entry's data to create the SchemaCompareObjectId
            const nameParts = value.sourceValue || value.targetValue;
            if (nameParts && nameParts.length > 0) {
                result.push({
                    nameParts: nameParts,
